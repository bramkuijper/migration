...
void postbreeding_dynamics(int t)
{
    ...
    for (int i = 0; i < summer_pop; ++i)
    {
		pdisperse = pow(1 + exp(- 0.5 * (SummerPop[i].psi_b[0] + SummerPop[i].psi_b[1]) 
			* (SummerPop[i].resources - 0.5 * (SummerPop[i].psi_a[0] + SummerPop[i].psi_a[1])) - 0.5 * (SummerPop[i].phi_b[0] + SummerPop[i].phi_b[1]) 
			* (((double) signal_pop_start / (signal_pop_start + winter_pop)) - 0.5 * (SummerPop[i].phi_a[0] + SummerPop[i].phi_a[1]))), -1);
		pdisperse = clamp(pdisperse, 0, 1);
