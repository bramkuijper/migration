...
{	
	...
	for (int i = 0; i < winter_pop; ++i)
    {
		pready_condition = pow(1 + exp(-0.5 * (WinterPop[i].psi_b[0] + WinterPop[i].psi_b[1]) 
			* (WinterPop[i].resources - 0.5 * (WinterPop[i].psi_a[0] + WinterPop[i].psi_a[1]))), -1);
		pready_condition = clamp(pready_condition, 0, 1);
		
		pready_social = pow(1 + exp(-0.5 * (WinterPop[i].phi_b[0] + WinterPop[i].phi_b[1]) 
