...
{	
    ...
    {		
		// Randomly assigning a migration cost based on the distribution of costs incurred by comparable non-control population
		
		double P = uniform(rng_r);
		double total = ones + twos + threes + fours + fives + sixes + sevens + eights;
		eights = eights/total;
		sevens = (eights + sevens)/total;
		sixes = (sevens + sixes)/total;
		fives = (sixes + fives)/total;
		fours = (fives + fours)/total;
		threes = (fours + threes)/total;
		twos = (threes + twos)/total;
		...
		if (P < eights)
		{
			...
			NFlock_fake -=1;
			assert(NFlock_fake = 1);
		}
		
		assert(NFlock_fake >= 1);
		
		SummerPop[i].flock_size = NFlock;
		SummerPop[i].cost = migration_cost(NFlock_fake);
